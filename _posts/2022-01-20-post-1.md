...
def country_diff(country, year_begin, year_end, month, **kwargs):
    ...
    title = "Regresstion plot in {a} for stations in {b}, years {c} - {d}"\
    .format(a = month, b = country, c = year_begin, d = year_end)
...
```python
fig = country_diff("India", 1980, 2020, 6)
fig.show()
```
...
In this data visualization, I will use Plotly package to plot a line plot to show the average temperature variation in any two stations in a country for different years.
...
def country_diff(country, station_1, station_2, year_begin, year_end, **kwargs):
    ...
    df["Average Temperature(°C)"] = df.groupby(["Station", "Year"])["Temp"].transform(np.mean)
    
    # plot's title
    title = "Average temperature change at Station {a} and {b} in {c}, years {d} - {e}"\
    .format(a = station_1, b = station_2, c = country, d = year_begin, e = year_end)
